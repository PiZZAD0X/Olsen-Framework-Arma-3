...
_ID = _unit addEventHandler ["FiredMan",{
	...
	allUnits select {(_x distance2d _firer <= VCOM_HEARINGDISTANCE) && (!isplayer _x)} apply {

		_distance = _x distance _firer;
		_travelTime = _distance / 343;//not using distance2d since that would be odd
		_revealValue = linearConversion [200,VCOM_HEARINGDISTANCE,_distance,4,VCOM_HEARINGMININCREASE];
