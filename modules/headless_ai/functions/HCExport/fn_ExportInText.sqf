...
private _type2 = format ["%1",(toLower _type)];
...
switch (_type2) do {
	case "object": {
		...
		{
			private _isUnit = _x isKindOf "Man";
			private _isVehicle = _x isKindOf "LandVehicle" || _x isKindOf "Ship" || _x isKindOf "Air";
	...
	case "marker": {
		...
		private _allMarkerType = ["ICON","RECTANGLE","ELLIPSE"];
	...
	case "groups": {
		...
		private _waypoints = switch _selectType do {
				...
				if !(_group in _addedGroups) then {
					...
					_textGrUs = "";
					{
						if !(_x in _addedUnits) then {
							...
							private _inNoVeh = _veh isEqualTo _x;
...

								{
									if !(_x in _addedUnits) then {
										if (_x in _units) then {
											...
											private _init = ["this", "_this", ((_x get3DENAttribute "Init") select 0)] call HC_fnc_ReplaceText;
											private _role = assignedVehicleRole _x;
											if (_role isEqualTo []) then { _role = ["cargo"]; };

											private _selNveh = if (_x in _waitUnits) then { _vehWaitUnits select (_waitUnits find _x) }else{
											...
											if !(_x in _waitUnits) then {
												...
												private _i = _waitUnits pushBack _x;
					...
					if !(_textGrUs isEqualTo "") then {
						...
						private _textGrWp = "";
						{
							...
							if (_grWt isEqualTo _group) then {
								...
								private _addonW = "";
								private _typeW = (_x get3DENAttribute "itemClass") select 0;
								...
								private _formationWt = _allFormations select ((_x get3DENAttribute "Formation") select 0);
								private _speedModeWt = _allSpeedModes select ((_x get3DENAttribute "speedMode") select 0);
