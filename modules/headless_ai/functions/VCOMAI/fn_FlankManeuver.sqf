...
if (isNull _myNearestEnemy) exitWith
{
	...
	if ((count (waypoints (group _Unit))) < 2) then
	{
		...
		_this spawn VCOMAI_fnc_FlankManeuver;	
	};
...
sleep 0.25;
if (_myEnemyPos isEqualTo [0,0,0]) exitWith {_VCOM_Flanking = false;[_Unit,_VCOM_Flanking] spawn VCOMAI_fnc_FlankManeuver;_VCOM_Flanking = true;};
