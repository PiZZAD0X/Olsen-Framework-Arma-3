...
if ((_Unit getVariable ["VCOM_NOPATHING_Unit",false]) || {(_Unit getVariable ["VCOM_GARRISONED",false])} || {(_Unit getVariable ["VCOM_NOAI",false])}) exitWith {if (VCOM_AIDEBUG isEqualTo 1) then {systemChat format ["Exited ClosestAllyWarn2..: %1",_UnitGroup];};};

private _ArrayOrg = _Unit call VCOMAI_fnc_FriendlyArray;
...
private _Array2 = _Killer call VCOMAI_fnc_FriendlyArray;
{
		...
		if (_RespondCount < _EnemyCount) then
		{
			...
			if (!(isNil "_CheckStatus2") && {!(_x getVariable ["VCOM_NOPATHING_Unit",false])} && {!(_x getVariable ["VCOM_GARRISONED",false])} && {"ItemRadio" in _CheckStatus2} && {!((group _x) getVariable ["VCOM_MOVINGTOSUPPORT",false])}) then 
			{
						...
						if ((count (waypoints _group)) < 2) then 
						{
							...
							private _WaypointCheck = _group call VCOMAI_fnc_Waypointcheck;
