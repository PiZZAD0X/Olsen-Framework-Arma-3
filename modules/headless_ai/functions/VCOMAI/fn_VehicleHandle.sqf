	...
	if (_CargoCount > 0) then 
	{
			if ((getPos _Vehicle select 2) < 3 && {(_myNearestEnemy distance _Driver) < 600}) then 
			{
				...
				_CargoList spawn {sleep 120;_this allowGetIn true;};
				{
					...
					unassignVehicle _U;
					sleep 1;
					//[_x,false,false,false,false] spawn VCOMAI_fnc_MoveToCover;
					...
					[_U,false,false,false,false] spawn VCOMAI_fnc_MoveToCover;
				} foreach _CargoList;			
		...
		{
				[_Driver,false,false,(_Driver getvariable ["VCOMAI_StartedInside",false]),false] spawn VCOMAI_fnc_FlankManeuver;
