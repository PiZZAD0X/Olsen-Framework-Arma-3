...
params ["_unit", "_loadoutType", "_items", "_randomPick", ["_valuesArray", [], [[]]], ["_weightsArray", [], [[]]], ["_badLoadout", false, [false]]];
...
for "_i" from 1 to _itemCount step 2 do {
	...
	if !(_itemWeight isEqualType "SCALAR") exitWith {
		_badLoadout = true;
	...
	{
		if !(_x isEqualType "ARRAY") exitWith {
			_badLoadout = true;
